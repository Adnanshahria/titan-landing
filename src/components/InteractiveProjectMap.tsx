    ...
    <div className="relative w-full">
      <div className="relative w-full max-w-2xl mx-auto">
        <div className="relative">
          <svg viewBox="0 0 1000 1000" className="w-full h-auto" style={{ filter: "drop-shadow(0 4px 20px hsl(var(--navy) / 0.1))" }}>
            <image href="/bangladesh-map.svg" x="0" y="0" width="1000" height="1000" opacity="0.25" />
            {[
              ...
              <text key={div.name} x={div.x} y={div.y} textAnchor="middle" fill="hsl(var(--navy) / 0.2)" fontSize="28" fontWeight="700" fontFamily="'Abril Fatface', serif" className="select-none pointer-events-none">
                {div.name}
              </text>
            ))}
            {filtered.map((pin) => {
              ...
              return (
                <g key={pin.id}>
                  <circle cx={pos.x} cy={pos.y} r={isHighlighted || isHovered ? 25 : 15} fill="hsl(var(--orange) / 0.2)" className="animate-pulse" />
                  <circle cx={pos.x} cy={pos.y} r={isHighlighted ? 16 : 12} fill="hsl(var(--orange))" stroke="hsl(var(--secondary-foreground))" strokeWidth="3" className="cursor-pointer" style={{ filter: isHighlighted ? "drop-shadow(0 0 8px hsl(var(--orange) / 0.6))" : "drop-shadow(0 2px 4px rgba(0,0,0,0.3))", transition: "all 0.3s ease" }} />
                  <circle cx={pos.x} cy={pos.y} r={isHighlighted ? 5 : 4} fill="hsl(var(--secondary-foreground))" />
                </g>
              );
            })}
          </svg>

          <div className="absolute inset-0" onClick={() => { if (tappedPin) setTappedPin(null); }} onTouchStart={(e) => { if (tappedPin && !(e.target as HTMLElement).closest("[data-map-pin]")) setTappedPin(null); }}>
            {filtered.map((pin) => {
              const pos = geoToSvg(pin.latitude, pin.longitude);
              return (
                <a key={pin.id} data-map-pin href={pin.project_slug ? `/project/${pin.project_slug}` : undefined} className="absolute z-10 w-10 h-10 -translate-x-1/2 -translate-y-1/2 cursor-pointer" style={{ left: `${pos.x / 10}%`, top: `${pos.y / 10}%` }} onMouseEnter={() => setHoveredPin(pin)} onMouseLeave={() => setHoveredPin(null)} onClick={(e) => handlePinTap(pin, e)} onTouchEnd={(e) => handlePinTap(pin, e)} />
              );
            })}
          </div>
                ...
                <div className="bg-card rounded-xl shadow-2xl border border-steel/20 overflow-hidden w-60">
                  {getPinImage(activeTooltipPin) && (
                    <div className="h-32 overflow-hidden relative">
                      <img src={getPinImage(activeTooltipPin)} alt={activeTooltipPin.project_name} className="w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                      <span className="absolute bottom-2 left-2 bg-gradient-to-r from-orange to-orange-glow text-secondary-foreground text-[9px] font-heading font-semibold px-2 py-0.5 rounded-full uppercase">{activeTooltipPin.category}</span>
                    </div>
                  )}
                  <div className="p-3">
                    <h4 className="font-heading text-xs font-bold text-foreground uppercase leading-tight line-clamp-2">{activeTooltipPin.project_name}</h4>
                    ...
                    {tappedPin && tappedPin.project_slug && (
                      <Link to={`/project/${tappedPin.project_slug}`} className="block mt-2 text-center bg-gradient-to-r from-orange to-orange-glow text-secondary-foreground text-[10px] font-heading font-semibold py-1.5 rounded-lg uppercase tracking-wider" onClick={() => setTappedPin(null)}>
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="flex items-center gap-1.5 bg-card/90 backdrop-blur-sm px-3 py-1.5 rounded-full border border-steel/15 shadow-sm">
            ...
            <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-heading font-semibold">{filtered.length} {t("projects.across")}</span>
          </div>
